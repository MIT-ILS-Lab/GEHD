...
def sample_MN_pair(X, n_MN, option=0):
    ...
    for i in numba.prange(n):
        for j in range(n_MN):
            ...
            if option == 4:  # Precomputed distances
                for t in range(sampled.shape[0]):
                    dist_list[t] = X[i, sampled[t]]  # Use distance matrix directly
            else:
                for t in range(sampled.shape[0]):
...
def find_neighbors(X, n_neighbors):
    n = X.shape[0]
    knn_distances = np.zeros((n, n_neighbors), dtype=np.float32)
    nbrs = np.zeros((n, n_neighbors), dtype=np.int32)
    for i in numba.prange(n):
        neighbors_indices = np.argsort(X[i, :])[1 : n_neighbors + 1]
        nbrs[i, :n_neighbors] = neighbors_indices
        knn_distances[i, :n_neighbors] = X[i, neighbors_indices]
    return nbrs, knn_distances
...
def preprocess_X(X, distance, apply_pca, verbose, seed, high_dim, low_dim):
    ...
    if distance == "precomputed":
        ...
        xmax = 0  # placeholder
        xmean = 0  # placeholder
        print_verbose("X is precomputed distance matrix", verbose)
        X = X.astype(np.float32)
    elif distance != "hamming" and high_dim > 100 and apply_pca:
        xmin = 0  # placeholder
        xmax = 0  # placeholder
...
def preprocess_X_new(X, distance, xmin, xmax, xmean, tsvd, apply_pca, verbose):
    ...
    Handles the "precomputed" distance case.
    """
    _, high_dim = X.shape  # This will work even if X is a distance matrix (n x n)

    if distance == "precomputed":  # No preprocessing needed for precomputed distances
...
def distance_to_option(distance="euclidean"):
    ...
    if distance == "euclidean":
        ...
        raise NotImplementedError(
            ...
            + "angular, hamming or precomputed is not supported"
        )
    return option
...
def generate_extra_pair_basis(
    basis,
    X,  # X can be either features or distance matrix
    n_neighbors,
    distance="euclidean",  # Can be 'precomputed'
    verbose=True,
):
    """Generate pairs connecting extra data to the basis.  Handles precomputed distances."""
    npr = X.shape[0]

    if distance == "precomputed":  # Precomputed distances
        n = basis.shape[0]  # basis is the distance matrix here
        assert X.shape == (
            npr,
            n,
        ), "X must have shape (npr, n) when distance is 'precomputed'"

        nbrs = np.zeros((npr, n_neighbors), dtype=np.int32)
        knn_distances = np.zeros((npr, n_neighbors), dtype=np.float32)

        for i in range(npr):
            neighbors_indices = np.argsort(X[i, :])[
                1 : n_neighbors + 1
            ]  # Corrected indices
            nbrs[i, :] = neighbors_indices
            knn_distances[i, :] = X[i, neighbors_indices]  # Get the distances
        print_verbose("Found nearest neighbor", verbose)

    else:  # Original method (feature vectors)
        n, dim = basis.shape
        assert X.shape[1] == dim, "Dimension mismatch between X and basis."
        ...
        if n - 1 < n_neighbors:
            logger.warning(
                "Sample size is smaller than n_neighbors. "
...
def generate_pair(X, n_neighbors, n_MN, n_FP, distance="euclidean", verbose=True):
    """Generate pairs for the dataset."""
    n, dim = X.shape
    # sample more neighbors than needed
    if n - 1 < n_neighbors:
        logger.warning(
            "Sample size is smaller than number of neighbor pairs"
    ...
    if n - 1 < n_FP:
        logger.warning(
            "Sample size is smaller than number of further pairs"
            " requested. n_FP will be reduced."
        )
    n_FP = min(n_FP, n - 1)
    if n - 1 < n_MN:
        logger.warning(
            "Sample size is smaller than number of mid-near pairs"
    ...
    if option == 4:  # Precomputed distances
        print_verbose("Using precomputed distances.", verbose)
        nbrs, knn_distances = find_neighbors(X, n_neighbors)  # numba accelerated
        tree = None
    else:
        tree = AnnoyIndex(dim, metric=distance)
        if _RANDOM_STATE is not None:
...
def generate_pair_no_neighbors(
    X, n_neighbors, n_MN, n_FP, pair_neighbors, distance="euclidean", verbose=True
):
...
def pacmap(
    ...
    if isinstance(Yinit, np.ndarray):
        ...
        raise ValueError(
            (
                f"The argument init is of the type {type(Yinit)}. "
...
class PaCMAPChild(BaseEstimator):
    ...
    distance: string, default="euclidean"
        Distance metric used for high-dimensional space. Allowed metrics include euclidean, manhattan, angular, hamming, precomputed.
    ...
    def __init__(
        ...
        self.lr = lr
        self.num_iters = (
            num_iters if hasattr(num_iters, "__len__") else (100, 100, num_iters)
        )
        ...
        if self.distance == "precomputed" and apply_pca:
            raise ValueError(
                "apply_pca = True for precomputed distance is not supported."
            )
    ...
    def decide_num_pairs(self, n):
        ...
        if n - 1 < self.n_FP:
            logger.warning(
                "Sample size cannot accommodate number of further pairs"
                " requested. n_FP will be reduced."
            )
    ...
    def fit(self, X, init=None, save_pairs=True):
        ...
        if self.distance == "precomputed" and init != "random":
            raise ValueError(
                "Initialization must be random for precomputed distance matrix."
            )
    ...
    def transform(self, X, basis=None, init=None, save_pairs=True):
        ...
        raise ValueError(
            "The `transform` method is not supported in the current version of PaCMAPChild."
        )
    ...
    def sample_pairs(self, X, save_tree):
        ...
        if self.pair_neighbors is None:
            ...
            print_verbose("Using user provided nearest neighbor pairs.", self.verbose)
            ...
            self.pair_neighbors, self.pair_MN, self.pair_FP = (
                generate_pair_no_neighbors(
                    X,
                    self.n_neighbors,
                    self.n_MN,
                    self.n_FP,
