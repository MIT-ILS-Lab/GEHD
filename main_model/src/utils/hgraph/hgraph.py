...
from main_model.src.utils.hgraph.modules.decide_edge_type import *
...
def avg_pool(
    ...
    data_torch_geometric = torch_geometric.data.Data(
        x=data.x, edge_index=data.edge_index
    )
    new_data = torch_geometric.nn.avg_pool(
        cluster, data_torch_geometric, transform=transform
    )
...
def avg_pool_maintain_old(
    ...
    new_layer = torch_geometric.nn.avg_pool(
        cluster, data_torch_geometric, transform=transform
    )
...
def pooling(data: Data, size: float, normal_aware_pooling):
    ...
    if normal_aware_pooling == False:
        ...
        n_size = (
            size * 3.0
        )  # a hyper parameter, controling "how important the normal vecs are, compared to xyz coords"
        size = [size, size, size, n_size, n_size, n_size]

    edges = data.edge_index
    cluster = torch_geometric.nn.voxel_grid(
        pos=x, size=size, batch=None, start=[-1, -1, -1.0], end=[1, 1, 1.0]
    )
    ...
    for i in range(
        int(mapping.shape[0])
    ):  # maybe some optimization here to remove the for loop
...
class HGraph:
        ...
        for i in range(self.depth + 1):
            if i == 0:
                ...
                if original_graph.edge_attr == None:
                    edges = (
                        original_graph.x[original_graph.edge_index[0]]
                        - original_graph.x[original_graph.edge_index[1]]
                    )
                    edges_attr = decide_edge_type_distance(
                        edges, return_edge_length=False
                    )
            ...
            clst = pooling(
                graphtree[i - 1],
                self.smallest_grid * (2 ** (i - 1)),
                normal_aware_pooling=self.normal_aware_pooling,
            )
            new_graph = avg_pool(cluster=clst, data=graphtree[i - 1], transform=None)
            new_graph = add_self_loop(new_graph)
            # assign edge type
            edges = (
                new_graph.x[new_graph.edge_index[0]]
    ...
    def merge_hgraph(self, original_graphs: list[HGraph], debug_report=False):
        ...
        """
        assert (
            len(self.cluster) == 0 and len(self.treedict) == 0
        ), "please call this function on a new instance"
        assert (
            original_graphs.__len__() == self.batch_size
        ), "please make sure the batch size is correct"
        ...
        for d in range(self.depth + 1):
            ...
            # construct new graph
            temp_data = Data(
                x=torch.cat(graphtrees_x, dim=0),
        ...
        if debug_report == True:
            ...
            for d in range(self.depth + 1):
                ...
                for i in range(original_graphs.__len__()):
                    num_edges_before += (
                        original_graphs[i].treedict[d].edge_index.shape[1]
                    )
