...
from main_model.src.utils.hgraph.hgraph import HGraph
from main_model.src.utils.hgraph.modules.modules import (
    Conv1x1BnRelu,
    Conv1x1Bn,
    GraphConvBnRelu,
    GraphConvBn,
...
class GraphResBlocks(torch.nn.Module):
    def __init__(
        self, in_channels, out_channels, resblk_num, bottleneck=4, resblk=GraphResBlock
    ):
        super().__init__()
        ...
        self.resblks = torch.nn.ModuleList(
            [
                resblk(channels[i], channels[i + 1], bottleneck=bottleneck)
