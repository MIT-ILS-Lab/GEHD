...
from main_model.src.utils.hgraph.hgraph import Data, HGraph, avg_pool
...
class MyConvOp(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_attr):
        ...
        if self.include_distance == True:
            abs_dist = torch.norm(
                x_i[..., -3:] - x_j[..., -3:], dim=1, keepdim=True
            )  # shape: [E, 1]
...
class MyConv(nn.Module):
    ...
    def __init__(
        ...
        self.include_distance = (
            True  # default_settings.get_global_value("include_distance")
        )
        if self.include_distance == False:
            self.conv = MyConvOp(in_channels, out_channels, include_distance=False)
        else:
            self.conv = MyConvOp(
                in_channels + 3 + 3 + 1, out_channels, include_distance=True
            )
...
class PoolingGraph(nn.Module):
    ...
    def forward(self, x: torch.Tensor, hgraph: HGraph, depth: int):
        ...
        out = avg_pool(
            cluster=cluster, data=Data(x=x, edge_index=torch.zeros([2, 1]).long())
        )  # fake edges XD
        return out.x
