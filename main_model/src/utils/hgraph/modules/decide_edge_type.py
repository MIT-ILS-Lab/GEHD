...
def decide_edge_type_distance(
    vec: torch.tensor, method="predefined", return_edge_length=True
):
    """
    ...
    if method == "predefined":
        return decide_edge_type_predefined_distance(
            vec, return_edge_length=return_edge_length
        )
    else:
        raise NotImplementedError


def decide_edge_type_predefined_distance(
    vec: torch.tensor, epsilon=0.00001, return_edge_length=True
):
    """
    ...
    assert edge_type.max() == len(
        dist_threshold
    )  # there must be edges indexed N, since self-loop exists in all meshes
    assert (
        edge_type.min() >= 0
    )  # note edge_type indexed N-1 may not exists, since edges of that length may not exists in this mesh
