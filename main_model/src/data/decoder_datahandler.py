...
class LEHDEnv:
    ...
    def sampling_subpaths(self, problems, solution, length_fix=False):
        ...
        temp_index_torch_2 = torch.from_numpy(temp_index_2).long().to(self.device)
    ...
    def step(
        self, selected, selected_student, selected_flag_teacher, selected_flag_student
    ):
    ...
    def drawPic_VRP(
        ...
        for i in range(len(tour) - 1):
            ...
            plt.plot(
                start, end, color=colors[count], linewidth=3
            )  # ,linestyle ="dashed"

            plt.scatter(arr[tour[i], 0], arr[tour[i], 1], color="gray", linewidth=2)
            plt.scatter(
                arr[tour[i + 1], 0], arr[tour[i + 1], 1], color="gray", linewidth=2
            )
    ...
    def valida_solution_legal(self, problem, solution, capacity_=50):
        ...
        capacitys = {100: 50, 200: 80, 500: 100, 1000: 250}
        ...
        index = torch.arange(sub_tours_padding.shape[0])[:, None].repeat(
            1, sub_tours_padding.shape[1]
        )
